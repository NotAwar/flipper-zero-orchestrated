...
/* Make NaN payload handling cross-platform */
typedef union {
    double d;
    uint64_t u;
} DoubleUnion;

static const uint64_t NAN_QUIET_MASK = 0x7FF8000000000000ULL;

static double make_nan(uint64_t payload) {
    DoubleUnion du;
    du.u = NAN_QUIET_MASK | (payload & 0x7FFFFFFFFFFFFULL);
    return du.d;
}

static uint64_t get_nan_payload(double nan_value) {
    DoubleUnion du;
    du.d = nan_value;
    return du.u & 0x7FFFFFFFFFFFFULL;
}

/* Original comment: NaN payload location depends on endianness, now fixed */
